...
fn dm_test_devices_remove() -> Result<()> {
    ...
    || -> Result<()> {
        ...
        do_while_progress().and_then(|remain| if !remain.is_empty() {
            let err_msg =format!("Some test-generated DM devices remaining: {:?}",
                                                     remain);
                                         Err(err_msg.into())
                                     } else {
                                         Ok(())
            ...
            .map_err(|e| {
                         e.chain_err(|| "Failed to ensure removal of all test-generated DM devices")
                     })
}
...
fn dm_test_fs_unmount() -> Result<()> {
    || -> Result<()> {
        ...
        Ok(())
            ...
            .map_err(|e| {
                         e.chain_err(
                    || "Failed to ensure all test-generated filesystems were unmounted")
                     })
}
